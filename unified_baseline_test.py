...
def test_single_method_model(method_name, model_name, model, testset, device, num_samples=30):
    ...
    # 获取参数
    if method_name == 'JSMA':
        params = UNIFIED_CONFIG['JSMA']
        print(f"参数: max_pixels={params['max_pixels']}, theta={params['theta']}")
    else:  # SparseFool
        params = UNIFIED_CONFIG['SparseFool']
        print(f"参数: max_iterations={params['max_iterations']}, lambda_={params['lambda_']}")
    
    # 选择样本（固定随机种子）
    np.random.seed(UNIFIED_CONFIG['random_seed'])
    torch.manual_seed(UNIFIED_CONFIG['random_seed'])
    ...
    for i, (idx, image, label) in enumerate(tqdm(selected_samples, desc=f"{method_name}")):
    ...
    for model_name in models:
        ...
        for method_name in methods:
            result = test_single_method_model(
                ...
                testset=testset,
                device=device,
                num_samples=UNIFIED_CONFIG['test_samples']
            )
            all_results.append(result)
            
            # 每次测试后保存（防止意外中断）
            output_dir = Path('results/unified_baseline')
