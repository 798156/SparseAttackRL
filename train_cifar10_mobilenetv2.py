...
def main():
    print("=" * 80)
    print("🚀 训练CIFAR-10 MobileNetV2 - 预训练微调方式")
    print("=" * 80)
    ...
    # 使用ImageNet预训练权重
    print("\n🔧 创建MobileNetV2模型（使用ImageNet预训练权重）...")
    model = torchvision.models.mobilenet_v2(weights='IMAGENET1K_V1')
    
    # MobileNetV2的分类器在model.classifier[1]
    # 原始：Linear(1280, 1000) → 修改为：Linear(1280, 10)
    model.classifier[1] = nn.Linear(model.classifier[1].in_features, 10)
    model = model.to(device)
    
    print(f"✅ MobileNetV2加载完成")
    print(f"   输入特征数: {model.classifier[1].in_features}")
    print(f"   输出类别数: 10")
