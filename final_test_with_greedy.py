...
CONFIG = {
    ...
    'Greedy': {
        'max_pixels': 10,
        'step_size': 0.2  # 诊断验证：90% ASR, L0=3.89
    },
...
def main():
    """主函数"""
    print("\n" + "="*80)
    print("🔬 完整Baseline测试 - 包含Greedy Attack")
    print("="*80)
    print("\n💡 统一参数配置:")
    print(f"  JSMA: max_pixels={CONFIG['JSMA']['max_pixels']}, theta={CONFIG['JSMA']['theta']}")
    print(f"  SparseFool: max_iterations={CONFIG['SparseFool']['max_iterations']}, lambda_={CONFIG['SparseFool']['lambda_']}")
    print(f"  Greedy: max_pixels={CONFIG['Greedy']['max_pixels']}, step_size={CONFIG['Greedy']['step_size']}")
    print(f"  样本数: {CONFIG['test_samples']}/模型")
    print(f"\n实验规模: 3模型 × 3方法 × 30样本 = 270个测试")
    print(f"预计时间: < 10分钟\n")
    ...
    summary = {
        ...
        'device': str(device),
        'note': 'Complete baseline with Greedy Attack',
        'results': all_results
    }
    ...
    print("  - 3种方法（JSMA, SparseFool, Greedy）")
    print("  - 每个组合30个样本")
    print("  - 总计270个测试")
    print("  - 统一的参数配置")
